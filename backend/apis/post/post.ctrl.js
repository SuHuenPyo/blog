...
//나의 최신글 가져오기  필수 m_id
const myRecent = async (req, res, next) => {


  logger.info(`[GET /myRecent] ${req.ip} is access`);
  console.log("[index] DB pool current Count == " + pool.pool._allConnections._tail);

  const page = req.query?.page || 1; //보내줄 해당 페이지 넘버. query로 값이 넘어 오지 않으면 맨처음 넘버 1로 보내줌
  const rows = req.query?.rows || 16; //페이지당 요청 개수

  const memberId = req.query?.m_id;

  if(!memberId) return res.status(400).send('유저 아이디가 비었습니다.');
  ...
  try {
    ...
    dbcon = await pool.getConnection(async (conn) => conn);
    sql =`SELECT COUNT(*) as cnt FROM boards WHERE m_id=?`;
    [result] = await dbcon.query(sql, [memberId]);
    totalCount = result[0].cnt ;


    

    if(result[0].cnt == 0) return res.status(204).json({});
    result=null;
    ...
    members.userId as memberUserId, members.image as memberPic,
    members.name as memberName
    FROM boards, members WHERE ((boards.m_id = members.m_id) AND members.m_id = ?  AND (boards.b_mdate > date_add(now(), interval -7 day)) ) ORDER BY boardId DESC LIMIT ?,?`;

    
    result = await dbcon.query(sql, [ memberId, pagenationResult.offset, pagenationResult.listCount]);
...
const myContent = async (req, res, next) => {
  ...
  logger.info(`[GET /myContent] ${req.ip} is access`);
  console.log("[index] DB pool current Count == " + pool.pool._allConnections._tail);
  ...
  console.log("MyCONTENT :::::  " +JSON.stringify(req.query) );
  ...
  try {
    ...
    members.userId as memberUserId, members.image as memberPic,
    members.name as memberName
    FROM boards, members WHERE ((boards.m_id = members.m_id) AND members.m_id = ? ) ORDER BY boardId DESC LIMIT ?,?`;
...
module.exports = { create, index, detail, popular, recent, update, destroy, search, myRecent, myContent };
